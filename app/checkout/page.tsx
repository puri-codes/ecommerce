...
type PromoResult = { code: string; discount_percent: number; discount_amount: number; final_amount: number };
...
export default function CheckoutPage() {
  ...
  function removePromo() { setPromoResult(null); setPromoInput(''); setPromoError(''); }
    ...
    return (
      <div className="flex-1 flex items-center justify-center min-h-[70vh] px-4">
        <div className="max-w-md w-full text-center">
          ...
          <p className="text-[#696969] text-sm mb-1">Thank you, <span className="font-medium text-black">{formData.fullName}</span>.</p>
          ...
          <div className="bg-[#EDE735] px-5 py-4 mb-8 text-left">
            <div className="flex items-start gap-3">
              ...
              <p className="text-sm font-medium leading-relaxed">
                We will follow up on call to confirm your order and arrange delivery. Keep your phone
                reachable on <span className="font-bold">{formData.phone}</span>.
              </p>
            </div>
          </div>
          <Link href="/"><button className="bg-black text-white px-8 py-3 text-sm font-medium hover:bg-black/80 transition-colors">Continue Shopping</button></Link>
        </div>
      </div>
    );
  }
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div>
              ...
              <input type="tel" inputMode="numeric" required value={formData.phone}
                ...
                className={`w-full border-b py-2.5 text-sm outline-none transition-colors bg-transparent ${phoneError ? 'border-[#FA5D42]' : 'border-gray-300 focus:border-black'}`}
            ...
            <div>
              ...
              <textarea value={formData.address} onChange={e => setFormData(d => ({ ...d, address: e.target.value }))}
                rows={2} className="w-full border-b border-gray-300 py-2.5 text-sm outline-none focus:border-black transition-colors bg-transparent resize-none"
            ...
            <div className="bg-gray-50 border border-gray-100 px-4 py-3 flex items-start gap-3 mt-2">
              ...
              <p className="text-[13px] text-[#696969] leading-relaxed">
                After placing your order, our team will <strong className="text-black">follow up on call</strong> to confirm and arrange delivery.
              </p>
            </div>
            ...
            <button type="submit" disabled={items.length === 0 || busy}
              ...
              {busy ? 'Placing order…' : `Place Order · Rs. ${finalTotal.toLocaleString()}`}
            </button>

            {items.length === 0 && (
              <p className="text-center text-sm text-[#696969]">
                Your cart is empty. <Link href="/" className="underline hover:text-black">Browse products</Link>
              </p>
            )}
          </form>
        </div>
                  ...
                  <div key={item.id} className="flex gap-3">
                    {item.image && (
                      <img src={item.image} alt={item.productTitle}
                        className="w-14 object-cover bg-gray-100 shrink-0" style={{ height: '4.5rem' }} />
                    )}
                    ...
                    <div className="text-[13px] font-medium shrink-0">Rs. {(item.price * item.quantity).toLocaleString()}</div>
                  </div>
                ))}
                  ...
                  {promoResult ? (
                    <div className="flex justify-between text-[#027D48] font-medium">
                      <span className="flex items-center gap-1.5">
                        ...
                        <button onClick={removePromo} className="text-[#aaa] hover:text-black ml-1">
                    ...
                    <div className="pt-1">
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 border-b border-gray-200 py-1.5 text-sm outline-none focus:border-black bg-transparent font-mono placeholder:font-sans placeholder:text-[#aaa]"
                        />
                        <button type="button" onClick={applyPromo} disabled={!promoInput.trim() || promoLoading}
                          className="border border-gray-200 px-3 py-1.5 text-xs hover:border-black transition-colors disabled:opacity-40">
                          {promoLoading ? '…' : 'Apply'}
                        </button>
                      </div>
                      {promoError && <p className="mt-1 text-[11px] text-[#FA5D42]">{promoError}</p>}
                    </div>
                  )}

                  <div className="flex justify-between items-center text-base font-semibold text-black border-t border-gray-200 pt-3">
                    <span>Total</span>
