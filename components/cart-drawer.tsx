...
export function CartDrawer() {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex justify-end ${isOpen ? '' : 'pointer-events-none'}`}>
      <div className={`fixed inset-0 bg-black/50 backdrop-blur-sm transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={() => setIsOpen(false)} />
      
      <div className={`relative w-full max-w-[400px] bg-white h-full shadow-xl flex flex-col flex-shrink-0 z-10 transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex items-center justify-between p-6 border-b border-gray-100 pb-4">
          <h2 className="text-xl font-medium flex items-center gap-2">
            Your Cart <span className="bg-gray-100 text-sm px-2 py-0.5 rounded-full">{items.length}</span>
          </h2>
                  ...
                  <div className="flex-1 flex flex-col justify-between py-1">
                    <div>
                      <div className="flex justify-between items-start gap-4">
                        <Link href={`/product/${item.productId}`} onClick={() => setIsOpen(false)} className="font-medium text-sm hover:underline line-clamp-2">
                          {item.productTitle}
                        </Link>
        ...
        <div className="border-t border-gray-100 p-6 bg-white shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
          ...
          <Link href="/checkout" onClick={(e) => { if (items.length === 0) e.preventDefault(); else setIsOpen(false); }}>
            <button 
              ...
              className="w-full bg-black text-white py-4 font-medium hover:bg-black/90 transition-colors disabled:bg-[#888888] disabled:opacity-100 disabled:cursor-not-allowed"
            >
              Checkout
            </button>
          </Link>
